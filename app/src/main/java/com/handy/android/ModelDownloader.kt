...
class ModelDownloader(private val context: Context) {
    data class Model(
        ...
        /** Null means the catalog has not published an authenticated digest yet. */
    ...
    val availableModels: List<Model>
        get() = listOf(
            Model("small", "Whisper Small", "ggml-small.bin", "https://blob.handy.computer/ggml-small.bin"),
            Model("turbo", "Whisper Turbo", "ggml-large-v3-turbo.bin", "https://blob.handy.computer/ggml-large-v3-turbo.bin"),
        ).map { it.inDirectory(directory) }
